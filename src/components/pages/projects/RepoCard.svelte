<script lang="ts">
  import { pluralify } from '../../../utils/string';
...
{#snippet cardContent()}
  <article class="card">
    ...
    <div class="meta">
      {@render metaStat('star-count', repo.stargazers_count, 'star', starIcon)}
      {@render metaStat('fork-count', repo.forks_count, 'fork', forkIcon)}
      ...
      {#if repo.totalPRs !== null && repo.totalPRs > 0}
        {@render metaStat('pr-count', repo.totalPRs, 'pull request', prIcon)}
      {/if}
    </div>
